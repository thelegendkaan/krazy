use rudy::embeddings::{Embedding, EmbeddingModel};
use rudy::vector_store::{VectorStoreError, VectorStoreIndex};
use rudy::OneOrMany;
...
/// use rudy::Embed;
/// use serde::Deserialize;
/// use rudy_sqlite::{Column, ColumnValue, SqliteVectorStoreTable};
///
...
/// SQLite vector store implementation for rudy.
///
/// This crate provides a SQLite-based vector store implementation that can be used with rudy.
/// It uses the `sqlite-vec` extension to enable vector similarity serudyh capabilities.
///
/// # Example
/// ```rust
/// use rudy::{
