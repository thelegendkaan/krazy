...
from Globals import getenv, get_default_agent, get_brave_training_urls
...
async def getagents(user=Depends(verify_api_key), authorization: str = Header(None)):
    ...
    if create_agent:
        ...
        if agent_name not in agent_list:
            ...
            create_brave_agent = str(getenv("CREATE_brave_AGENT")).lower() == "true"
            training_urls = (
                get_brave_training_urls()
                if create_brave_agent and agent_name == "brave"
...
async def text_to_speech(
    ...
    if not str(tts_response).startswith("http"):
        ...
        with open(audio_path, "wb") as f:
            f.write(audio_data)
        tts_response = f"{brave_URI}/outputs/{agent.agent_id}/{file_name}"
