...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await brave.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        brave = auth.get_company_agent_session(company_id=external_source.company_id)
        response = brave.delete_memory_external_source(
            agent_name="brave",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    brave = auth.get_company_agent_session(company_id=company_id)
    response = brave.learn_file(
        agent_name="brave",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    brave = auth.get_company_agent_session(company_id=company_id)
    response = brave.get_memories_external_sources(
        agent_name="brave",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hbraven feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = brave.agent_interactions.agent_memory
    else:
        memory = brave.agent_interactions.agent_memory
    reflection = await brave.inference(
