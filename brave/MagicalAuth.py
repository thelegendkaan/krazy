...
- brave_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- brave_URI: URL to the brave server
...
def is_brave_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("brave_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    ...
    if brave_API_KEY:
        if authorization == brave_API_KEY:
        ...
        logging.error(
            "brave API Key is missing. Please set the brave_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("brave_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                brave = braveSDK(base_uri=getenv("brave_URI"))
                brave.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                brave.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                brave.add_agent(
                    agent_name="brave",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        brave = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> braveSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> braveSDK:
        if not company_id:
            company_id = self.company_id
        ...
        brave.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return brave
