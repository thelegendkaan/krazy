...
def execute_python_code(
    ...
    docker_image = "joshxt/safeexecute:main"
    docker_working_dir = f"/brave/WORKSPACE/{agent_id}/{conversation_id}"
    os.makedirs(docker_working_dir, exist_ok=True)
    host_working_dir = os.getenv("WORKING_DIRECTORY", "/brave/WORKSPACE")
...
class brave_actions(Extensions):
    """
    The brave Actions extension contains commands that allow the brave Agents to work with other agents and use pre-built chains to complete tasks.
    """
    ...
    async def replace_init_in_file(self, filename: str, new_init: str):
        ...
        return f"{brave_uri}/outputs/extensions/{filename}"
    ...
    async def generate_openapi_chain(
        ...
        """
        Generate an brave extension from an OpenAPI JSON URL
    ...
    async def convert_string_to_pydantic_model(
        ...
        response = self.ApiClient.prompt_agent(
            ...
            prompt_args={
                ...
                "schema": schema,
                "conversation_name": "brave Terminal",
            },
        )
