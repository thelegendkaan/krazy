...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await zenni.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        zenni = auth.get_company_agent_session(company_id=external_source.company_id)
        response = zenni.delete_memory_external_source(
            agent_name="zenni",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    zenni = auth.get_company_agent_session(company_id=company_id)
    response = zenni.learn_file(
        agent_name="zenni",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    zenni = auth.get_company_agent_session(company_id=company_id)
    response = zenni.get_memories_external_sources(
        agent_name="zenni",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hzennin feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = zenni.agent_interactions.agent_memory
    else:
        memory = zenni.agent_interactions.agent_memory
    reflection = await zenni.inference(
