...
- zenni_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- zenni_URI: URL to the zenni server
...
def is_zenni_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("zenni_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    ...
    if zenni_API_KEY:
        if authorization == zenni_API_KEY:
        ...
        logging.error(
            "zenni API Key is missing. Please set the zenni_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("zenni_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                zenni = zenniSDK(base_uri=getenv("zenni_URI"))
                zenni.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                zenni.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                zenni.add_agent(
                    agent_name="zenni",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        zenni = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> zenniSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> zenniSDK:
        if not company_id:
            company_id = self.company_id
        ...
        zenni.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return zenni
