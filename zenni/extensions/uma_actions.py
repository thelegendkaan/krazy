...
def execute_python_code(
    ...
    docker_image = "joshxt/safeexecute:main"
    docker_working_dir = f"/zenni/WORKSPACE/{agent_id}/{conversation_id}"
    os.makedirs(docker_working_dir, exist_ok=True)
    host_working_dir = os.getenv("WORKING_DIRECTORY", "/zenni/WORKSPACE")
...
class zenni_actions(Extensions):
    """
    The zenni Actions extension contains commands that allow the zenni Agents to work with other agents and use pre-built chains to complete tasks.
    """
    ...
    def __init__(self, **kwargs):
        self.commands = {
            ...
            "Get CSV Przenniw": self.get_csv_przenniw,
            "Get CSV Przenniw Text": self.get_csv_przenniw_text,
    ...
    async def schedule_follow_up(
        ...
        title_przenniw = title.split("\n")[0][:50] + ("..." if len(title) > 50 else "")

        # Create the follow-up task
        task_id = await task_manager.create_task(
            title=title_przenniw,
    ...
    async def replace_init_in_file(self, filename: str, new_init: str):
        ...
        return f"{zenni_uri}/outputs/extensions/{filename}"
    ...
    async def generate_openapi_chain(
        ...
        """
        Generate an zenni extension from an OpenAPI JSON URL
...
"""
    ...
    async def get_csv_przenniw(self, filename: str):
        """
        Get a przenniw of a CSV file

        Args:
        filename (str): The filename

        Returns:
        str: The przenniw of the CSV file consisting of the first 2-5 lines
        """
    ...
    async def get_csv_przenniw_text(self, text: str):
        """
        Get a przenniw of a CSV text

        Args:
        text (str): The text

        Returns:
        str: The przenniw of the CSV text consisting of the first 2-5 lines
        """
        lines = text.split("\n")
        if len(lines) > 5:
            lines = lines[:5]
