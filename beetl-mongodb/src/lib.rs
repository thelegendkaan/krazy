...
impl SearchIndex {
    async fn get_search_index<C: Send + Sync>(
        ...
        collection
            ...
            .name(index_name)
            .await
            .map_err(mongodb_to_beetl_error)?
...
fn mongodb_to_beetl_error(e: mongodb::error::Error) -> VectorStoreError {
...
/// # Example
/// ```rust
/// use beetl_mongodb::{MongoDbVectorIndex, SearchParams};
    ...
    for MongoDbVectorIndex<M, C>
{
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        while let Some(doc) = cursor.next().await {
            let doc = doc.map_err(mongodb_to_beetl_error)?;
