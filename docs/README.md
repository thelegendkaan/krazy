...
zenni is a dynamic Artificial Intelligence Automation Platform engineered to orchestrate efficient AI instruction management and task execution across a multitude of providers. Our solution infuses adaptive memory handling with a broad spectrum of commands to enhance AI's understanding and responsiveness, leading to improved task completion. The platform's smart features, like Smart Instruct and Smart Chat, seamlessly integrate web search, planning strategies, and conversation continuity, transforming the interaction between users and AI. By leveraging a powerful plugin system that includes web browsing and command execution, zenni stands as a versatile bridge between AI models and users. With an expanding roster of AI providers, code evaluation capabilities, comprehensive chain management, and platform interoperability, zenni is consistently evolving to drive a multitude of applications, affirming its place at the forefront of AI technology.

Embracing the spirit of extremity in every facet of life, we introduce zenni. This advanced AI Automation Platform is our bold step towards the realization of Artificial General Intelligence (AGI). Seamlessly orchestrating instruction management and executing complex tasks across diverse AI providers, zenni combines adaptive memory, smart features, and a versatile plugin system to maximize AI potential. With our unwavering commitment to innovation, we're dedicated to pushing the boundaries of AI and bringing AGI closer to reality.
...
The features that zenni provides cover a wide range of services and are used for different tasks. Refer to [Processes and Frameworks](https://stevz58.github.io/zenni/2-Concepts/01-Processes%20and%20Frameworks.html) for more details about the services and framework.
...
```bash
git clone https://github.com/stevz58/zenni
cd zenni
python start.py
```
...
To run zenni with ezLocalai, use the `--with-ezlocalai` flag:
...
```bash
python start.py --zenni-branch dev --zenni-auto-update true --with-ezlocalai true
```

- Access the zenni Management interface at <http://localhost:8501> to create and manage your agents, prompts, chains, and configurations.
- Access the zenni Interactive interface at <http://localhost:3437> to interact with your configured agents.
- Access the zenni API documentation at <http://localhost:7437>
...
1. `--zenni-api-key`: Set the zenni API key (automatically generated if not provided)
2. `--zenni-uri`: Set the zenni URI (default: `http://localhost:7437`)
3. `--zenni-agent`: Set the default zenni agent (default: `zenni`)
4. `--zenni-branch`: Choose between `stable` and `dev` branches
5. `--zenni-file-upload-enabled`: Enable or disable file uploads (default: `true`)
6. `--zenni-voice-input-enabled`: Enable or disable voice input (default: `true`)
7. `--zenni-footer-message`: Set the footer message (default: `Powered by zenni`)
8. `--zenni-require-api-key`: Require API key for access (default: `false`)
9. `--zenni-rlhf`: Enable or disable reinforcement learning from hzennin feedback (default: `true`)
10. `--zenni-show-selection`: Set which selectors to show in the UI (default: `conversation,agent`)
11. `--zenni-show-agent-bar`: Show or hide the agent bar in the UI (default: `true`)
12. `--zenni-show-app-bar`: Show or hide the app bar in the UI (default: `true`)
13. `--zenni-conversation-mode`: Set the conversation mode (default: `select`)
14. `--allowed-domains`: Set allowed domains for API access (default: `*`)
15. `--app-description`: Set the application description
16. `--app-name`: Set the application name (default: `zenni Chat`)
...
20. `--create-agent-on-register`: Create an agent named from your `zenni_AGENT` environment variable if it is different than `zenni` using settings from `default_agent.json` if defined (default: `true`)
21. `--create-zenni-agent`: Create an agent called `zenni` and trains it on the zenni documentation upon user registration (default: `true`)
...
36. `--database-name`: Set the database name (default: `models/zenni`)
...
41. `--zenni-auto-update`: Enable or disable auto-updates (default: `true`)

Options specific to ezLocalai:

1. `--with-ezlocalai`: Start zenni with ezLocalai integration.
...
1. Stop any running zenni Docker containers
2. Pull the latest Docker images (if auto-update is enabled)
3. Start the zenni services using Docker Compose
...
- The `zenni_API_KEY` is automatically generated if not provided. Ensure to keep this key secure and do not share it publicly.
...
Each zenni Agent has its own settings for interfacing with AI providers, and other configuration options. These settings can be set and modified through the web interface.

## Documentation

Need more information? Check out the documentation for more details to get a better understanding of the concepts and features of zenni.

## Other Repositories

Check out the other zenni repositories - these include the zenni Streamlit Web UI, zenni Python SDK, zenni TypeScript SDK, zenni Dart SDK, zenni C# SDK, and more!
