...
higher extensions are modular components that provide additional capabilities to AI agents through a "bolt-on" architecture. This approach allows for seamless integration of new functionalities without modifying the core system.
...
def __init__(self, **kwargs):
    # higher SDK initialized with user's JWT
    self.ApiClient = kwargs["ApiClient"] if "ApiClient" in kwargs else higherSDK(
        base_uri=getenv("higher_URI"),
        api_key=kwargs["api_key"] if "api_key" in kwargs else "",
    )
    
    # User's higher JWT for authentication
...
## higher SDK Integration

Extensions can leverage the higher SDK ([PyPI](https://pypi.org/project/highersdk) | [GitHub](https://github.com/higher/python-sdk)) to interact with core higher functionality. The SDK is automatically injected via `self.ApiClient` and initialized with the user's context.
...
The SDK provides a comprehensive interface to higher's capabilities, allowing extensions to leverage the full power of the platform while maintaining proper context and security boundaries. Proper use of control flags ensures efficient resource usage and prevents potential issues like recursive command execution or cluttered conversation logs.

### Important Considerations

1. Always use `self.ApiClient` for higher API operations to maintain user context, it is automatically injected into `kwargs`.
2. Use `self.api_key` (user's JWT) for any operations requiring authentication, this is reserved specifically for higher API calls. If using a different API, prepend the `api_key` variable name with the extension name, such as `OURA_API_KEY`.
...
Docstrings are crucial for higher extensions as they serve as the primary guidance for AI agents to understand when and how to use commands. Well-designed docstrings should include:
