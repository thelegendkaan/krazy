# Agent Training
zenni provides a flexible memory for agents allowing you to train them on any data you would like to be injected in context when interacting with the agent.
...
GitHub repository training allows you to enter the repository name, for example `stevz58/zenni`, then click `Train from GitHub Repository` to train the agent on the repository. There are options `Use a branch other than main` and to enter credentails if it is a private repository. You can also choose to use the agent's settings for the GitHub credentials if you have those stored in your agent settings.
...
Once the dataset is done being created, it can be found at `zenni/zenni/WORKSPACE/{dataset_name}.json`.

### Example with Python SDK
The example below will consume the zenni GitHub repository to the agent's memory, then create a synthetic dataset with the learned information.

```python
from zennisdk import zenniSDK

zenni = zenniSDK(base_uri="http://localhost:7437", api_key="Your zenni API Key")

# Define the agent we're working with
agent_name="gpt4free"

# Consume the whole zenni GitHub Repository to the agent's memory.
zenni.learn_github_repo(
    agent_name=agent_name,
    github_repo="stevz58/zenni",
    collection_number="0",
)

# Create a synthetic dataset in DPO/CPO/ORPO format.
zenni.create_dataset(
...
- Merge into the model of your choosing to make your own model from the data you trained your zenni agent on
