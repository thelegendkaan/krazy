...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await krazy.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        krazy = auth.get_company_agent_session(company_id=external_source.company_id)
        response = krazy.delete_memory_external_source(
            agent_name="krazy",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    krazy = auth.get_company_agent_session(company_id=company_id)
    response = krazy.learn_file(
        agent_name="krazy",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    krazy = auth.get_company_agent_session(company_id=company_id)
    response = krazy.get_memories_external_sources(
        agent_name="krazy",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hkrazyn feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = krazy.agent_interactions.agent_memory
    else:
        memory = krazy.agent_interactions.agent_memory
    reflection = await krazy.inference(
