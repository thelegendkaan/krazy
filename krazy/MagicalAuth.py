...
- krazy_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- krazy_URI: URL to the krazy server
...
def is_krazy_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("krazy_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    ...
    if krazy_API_KEY:
        if authorization == krazy_API_KEY:
        ...
        logging.error(
            "krazy API Key is missing. Please set the krazy_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("krazy_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                krazy = krazySDK(base_uri=getenv("krazy_URI"))
                krazy.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                krazy.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                krazy.add_agent(
                    agent_name="krazy",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        krazy = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> krazySDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> krazySDK:
        if not company_id:
            company_id = self.company_id
        ...
        krazy.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return krazy
