...
- luv_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- luv_URI: URL to the luv server
...
def is_luv_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("luv_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    ...
    if luv_API_KEY:
        ...
        logging.error(
            "luv API Key is missing. Please set the luv_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("luv_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                luv = luvSDK(base_uri=getenv("luv_URI"))
                luv.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                luv.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                luv.add_agent(
                    agent_name="luv",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        luv = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> luvSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> luvSDK:
        if not company_id:
            company_id = self.company_id
        ...
        luv.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return luv
