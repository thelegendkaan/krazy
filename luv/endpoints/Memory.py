...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await luv.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        luv = auth.get_company_agent_session(company_id=external_source.company_id)
        response = luv.delete_memory_external_source(
            agent_name="luv",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    luv = auth.get_company_agent_session(company_id=company_id)
    response = luv.learn_file(
        agent_name="luv",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    luv = auth.get_company_agent_session(company_id=company_id)
    response = luv.get_memories_external_sources(
        agent_name="luv",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hluvn feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = luv.agent_interactions.agent_memory
    else:
        memory = luv.agent_interactions.agent_memory
    reflection = await luv.inference(
