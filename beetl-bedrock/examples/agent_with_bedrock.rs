use beetl::{agent::AgentBuilder, completion::Prompt, loaders::FileLoader};
use beetl_bedrock::{
...
async fn partial_agent() -> AgentBuilder<beetl_bedrock::completion::CompletionModel> {
...
///
/// This example loads in all the rust examples from the beetl-core crate and uses them as\\
///  context for the agent
async fn loaders() -> Result<(), anyhow::Error> {
    let model = client().await.completion_model(AMAZON_NOVA_LITE);

    // Load in all the rust examples
    let examples = FileLoader::with_glob("beetl-core/examples/*.rs")?
        .read_with_path()
        .ignore_errors()
        .into_iter();
