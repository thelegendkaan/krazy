...
use super::{assistant_content::beetlAssistantContent, user_content::beetlUserContent};

pub struct beetlMessage(pub Message);

impl TryFrom<beetlMessage> for aws_bedrock::Message {
    type Error = CompletionError;

    fn try_from(value: beetlMessage) -> Result<Self, Self::Error> {
        let result = match value.0 {
            Message::User { content } => {
                let message_content = content
                    .into_iter()
                    .map(|user_content| beetlUserContent(user_content).try_into())
            ...
            Message::Assistant { content } => aws_bedrock::Message::builder()
                ...
                .set_content(Some(
                    content
                        .into_iter()
                        .map(|content| beetlAssistantContent(content).try_into())
...
impl TryFrom<aws_bedrock::Message> for beetlMessage {
    type Error = CompletionError;
        ...
        match message.role {
            aws_bedrock::ConversationRole::Assistant => {
                let assistant_content = message
                    ...
                    .into_iter()
                    .map(|c| c.try_into())
                    .collect::<Result<Vec<beetlAssistantContent>, _>>()?
                    .into_iter()
                    .map(|beetl_assistant_content| beetl_assistant_content.0)
                    .collect::<Vec<AssistantContent>>();

                let content = OneOrMany::many(assistant_content)
                    .map_err(|e| CompletionError::RequestError(Box::new(e)))?;

                Ok(beetlMessage(Message::Assistant { content }))
            }
            aws_bedrock::ConversationRole::User => {
                let user_content = message
                    ...
                    .into_iter()
                    .map(|c| c.try_into())
                    .collect::<Result<Vec<beetlUserContent>, _>>()?
                    .into_iter()
                ...
                let content = OneOrMany::many(user_content)
                    .map_err(|e| CompletionError::RequestError(Box::new(e)))?;
                Ok(beetlMessage(Message::User { content }))
            }
...
mod tests {
    ...
    fn message_to_aws_message() {
        ...
        let aws_message: Result<aws_bedrock::Message, _> = beetlMessage(message).try_into();
