...
use beetl::completion::{CompletionError, Message};
use beetl::message::{ContentFormat, DocumentMediaType, UserContent};
use beetl::OneOrMany;

pub struct AwsCompletionRequest(pub beetl::completion::CompletionRequest);
...
impl AwsCompletionRequest {
    ...
    pub fn messages(&self) -> Result<Vec<aws_bedrock::Message>, CompletionError> {
        ...
        full_history
            .into_iter()
            .map(|message| beetlMessage(message).try_into())
