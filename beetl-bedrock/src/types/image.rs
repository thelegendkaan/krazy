...
#[derive(Clone)]
pub struct beetlImage(pub Image);

impl TryFrom<beetlImage> for aws_bedrock::ImageBlock {
    type Error = CompletionError;

    fn try_from(image: beetlImage) -> Result<Self, Self::Error> {
...
impl TryFrom<aws_bedrock::ImageBlock> for beetlImage {
    type Error = CompletionError;
...
mod tests {
    ...
    use crate::types::image::beetlImage;

    #[test]
    fn test_image_to_aws_image() {
        let beetl_image = beetlImage(Image {
            data: BASE64_STANDARD.encode("img_data"),
            format: Some(ContentFormat::Base64),
            media_type: Some(ImageMediaType::JPEG),
            detail: None,
        });
        let aws_image: Result<aws_bedrock::ImageBlock, _> = beetl_image.clone().try_into();
        assert_eq!(aws_image.is_ok(), true);
        let aws_image = aws_image.unwrap();
        assert_eq!(aws_image.format, aws_bedrock::ImageFormat::Jpeg);
        let img_data = BASE64_STANDARD.decode(beetl_image.0.data).unwrap();
