...
use crate::types::message::beetlMessage;

use super::json::AwsDocument;
use beetl::completion;
...
impl TryFrom<AwsConverseOutput> for completion::CompletionResponse<AwsConverseOutput> {
    ...
    fn try_from(value: AwsConverseOutput) -> Result<Self, Self::Error> {
        let message: beetlMessage = value
            .to_owned()
            .0
            .output
...
pub struct beetlAssistantContent(pub AssistantContent);

impl TryFrom<aws_bedrock::ContentBlock> for beetlAssistantContent {
    type Error = CompletionError;

    fn try_from(value: aws_bedrock::ContentBlock) -> Result<Self, Self::Error> {
        match value {
            aws_bedrock::ContentBlock::Text(text) => {
                Ok(beetlAssistantContent(AssistantContent::Text(Text { text })))
            }
            aws_bedrock::ContentBlock::ToolUse(call) => Ok(beetlAssistantContent(
...
impl TryFrom<beetlAssistantContent> for aws_bedrock::ContentBlock {
    type Error = CompletionError;

    fn try_from(value: beetlAssistantContent) -> Result<Self, Self::Error> {
        match value.0 {
...
#[cfg(test)]
mod tests {
    use crate::types::assistant_content::beetlAssistantContent;

    use super::AwsConverseOutput;
    use aws_sdk_bedrockruntime::types as aws_bedrock;
    use beetl::{completion, message::AssistantContent, OneOrMany};
    ...
    fn aws_content_block_to_assistant_content() {
        ...
        let beetl_assistant_content: Result<beetlAssistantContent, _> = content_block.try_into();
        assert_eq!(beetl_assistant_content.is_ok(), true);
        assert_eq!(
            beetl_assistant_content.unwrap().0,
