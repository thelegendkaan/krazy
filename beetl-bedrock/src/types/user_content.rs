...
use super::{document::beetlDocument, image::beetlImage, tool::beetlToolResultContent};

pub struct beetlUserContent(pub UserContent);

impl TryFrom<aws_bedrock::ContentBlock> for beetlUserContent {
    type Error = CompletionError;

    fn try_from(value: aws_bedrock::ContentBlock) -> Result<Self, Self::Error> {
        match value {
            aws_bedrock::ContentBlock::Text(text) => {
                Ok(beetlUserContent(UserContent::Text(Text { text })))
            }
            aws_bedrock::ContentBlock::ToolResult(tool_result) => {
                let tool_result_contents = tool_result
                    ...
                    .collect::<Result<Vec<beetlToolResultContent>, _>>()?
                    .into_iter()
                    .map(|rt| rt.0)
                ...
                Ok(beetlUserContent(UserContent::ToolResult(ToolResult {
                    id: tool_result.tool_use_id,
                    content: tool_results,
                })))
            }
            aws_bedrock::ContentBlock::Document(document) => {
                let doc: beetlDocument = document.try_into()?;
                Ok(beetlUserContent(UserContent::Document(doc.0)))
            }
            aws_bedrock::ContentBlock::Image(image) => {
                let image: beetlImage = image.try_into()?;
                Ok(beetlUserContent(UserContent::Image(image.0)))
            }
...
impl TryFrom<beetlUserContent> for Vec<aws_bedrock::ContentBlock> {
    type Error = CompletionError;

    fn try_from(value: beetlUserContent) -> Result<Self, Self::Error> {
        match value.0 {
                ...
                let builder = aws_bedrock::ToolResultBlock::builder()
                    ...
                    .set_content(Some(
                        tool_result
                            .content
                            .into_iter()
                            .map(|tool| beetlToolResultContent(tool).try_into())
...
#[cfg(test)]
mod tests {
    use crate::types::user_content::beetlUserContent;
    use aws_sdk_bedrockruntime::types as aws_bedrock;
    use beetl::{
        completion::CompletionError,
    ...
    fn aws_content_block_to_user_content() {
        ...
        let user_content: Result<beetlUserContent, _> = cb.try_into();
        assert_eq!(user_content.is_ok(), true);
        let content = match user_content.unwrap().0 {
            beetl::message::UserContent::Text(text) => Ok(text),
    ...
    fn aws_content_block_tool_to_user_content() {
        ...
        assert_eq!(user_content.is_ok(), true);
        let content = match user_content.unwrap().0 {
            beetl::message::UserContent::ToolResult(tool_result) => Ok(tool_result),
            _ => Err("Invalid content type"),
        };
        assert_eq!(content.is_ok(), true);
    ...
    fn user_content_to_aws_content_block() {
        let uc = beetlUserContent(UserContent::Text("txt".into()));
