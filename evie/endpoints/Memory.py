...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await evie.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        evie = auth.get_company_agent_session(company_id=external_source.company_id)
        response = evie.delete_memory_external_source(
            agent_name="evie",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    evie = auth.get_company_agent_session(company_id=company_id)
    response = evie.learn_file(
        agent_name="evie",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    evie = auth.get_company_agent_session(company_id=company_id)
    response = evie.get_memories_external_sources(
        agent_name="evie",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hevien feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = evie.agent_interactions.agent_memory
    else:
        memory = evie.agent_interactions.agent_memory
    reflection = await evie.inference(
