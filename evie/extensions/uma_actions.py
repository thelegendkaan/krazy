...
def execute_python_code(
    ...
    docker_image = "joshxt/safeexecute:main"
    docker_working_dir = f"/evie/WORKSPACE/{agent_id}/{conversation_id}"
    os.makedirs(docker_working_dir, exist_ok=True)
    host_working_dir = os.getenv("WORKING_DIRECTORY", "/evie/WORKSPACE")
...
class evie_actions(Extensions):
    """
    The evie Actions extension contains commands that allow the evie Agents to work with other agents and use pre-built chains to complete tasks.
    """
    ...
    async def replace_init_in_file(self, filename: str, new_init: str):
        ...
        return f"{evie_uri}/outputs/extensions/{filename}"
    ...
    async def generate_openapi_chain(
        ...
        """
        Generate an evie extension from an OpenAPI JSON URL
