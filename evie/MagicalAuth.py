...
- evie_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- evie_URI: URL to the evie server
...
def is_evie_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("evie_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    ...
    if evie_API_KEY:
        ...
        logging.error(
            "evie API Key is missing. Please set the evie_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("evie_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                evie = evieSDK(base_uri=getenv("evie_URI"))
                evie.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                evie.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                evie.add_agent(
                    agent_name="evie",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        evie = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> evieSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> evieSDK:
        if not company_id:
            company_id = self.company_id
        ...
        evie.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return evie
