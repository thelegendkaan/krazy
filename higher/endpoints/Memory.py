...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await higher.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        higher = auth.get_company_agent_session(company_id=external_source.company_id)
        response = higher.delete_memory_external_source(
            agent_name="higher",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    higher = auth.get_company_agent_session(company_id=company_id)
    response = higher.learn_file(
        agent_name="higher",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    higher = auth.get_company_agent_session(company_id=company_id)
    response = higher.get_memories_external_sources(
        agent_name="higher",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hhighern feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = higher.agent_interactions.agent_memory
    else:
        memory = higher.agent_interactions.agent_memory
    reflection = await higher.inference(
