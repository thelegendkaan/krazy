...
from Globals import getenv, get_default_agent, get_higher_training_urls
...
async def getagents(user=Depends(verify_api_key), authorization: str = Header(None)):
    ...
    if create_agent:
        ...
        if agent_name not in agent_list:
            ...
            create_higher_agent = str(getenv("CREATE_higher_AGENT")).lower() == "true"
            training_urls = (
                get_higher_training_urls()
                if create_higher_agent and agent_name == "higher"
...
async def text_to_speech(
    ...
    if not str(tts_response).startswith("http"):
        ...
        with open(audio_path, "wb") as f:
            f.write(audio_data)
        tts_response = f"{higher_URI}/outputs/{agent.agent_id}/{file_name}"
