...
class Extensions:
    def __init__(
        ...
        self.ApiClient = (
            ApiClient
            if ApiClient
            else higherSDK(base_uri=getenv("API_URL"), api_key=api_key)
        )
    ...
    def get_extension_settings(self):
        ...
        if self.chains_with_args:
            settings["higher Chains"] = {}
            for chain in self.chains_with_args:
                chain_name = chain["chain_name"]
                chain_args = chain["args"]
                if chain_args:
                    settings["higher Chains"][chain_name] = {
                        "user_input": "",
    ...
    async def execute_command(self, command_name: str, command_args: dict = None):
        ...
        higher_server = getenv("higher_URI")
        injection_variables = {
            "user": self.user,
            "agent_name": self.agent_name,
            ...
            "output_url": f"{higher_server}/outputs/{self.agent_id}/{self.conversation_id}/",
    ...
    def get_extensions(self):
        ...
        for command_file in command_files:
            ...
            if extension_name == "higher Actions":
                extension_name = "higher Actions"
            commands.append(
                {
        ...
        # Add higher Chains as an extension only if chains_with_args is initialized
        if hasattr(self, "chains_with_args") and self.chains_with_args:
            chain_commands = []
            for chain in self.chains_with_args:
                chain_commands.append(
                    {
                        ...
                        "description": f"Execute higher Chain: `{chain['chain_name']}`.  The assistant can use the 'user_input' field as a place to summarize what the user needs when running the command.",
