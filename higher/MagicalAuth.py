...
- higher_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- higher_URI: URL to the higher server
...
def is_higher_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("higher_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    higher_API_KEY = getenv("higher_API_KEY")
    authorization = str(authorization).replace("Bearer ", "").replace("bearer ", "")
    if higher_API_KEY:
        if authorization == higher_API_KEY:
        ...
        logging.error(
            "higher API Key is missing. Please set the higher_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("higher_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                higher = higherSDK(base_uri=getenv("higher_URI"))
                higher.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                higher.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                higher.add_agent(
                    agent_name="higher",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        higher = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> higherSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> higherSDK:
        if not company_id:
            company_id = self.company_id
        ...
        higher.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return higher
