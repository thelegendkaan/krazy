...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await gravy.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        gravy = auth.get_company_agent_session(company_id=external_source.company_id)
        response = gravy.delete_memory_external_source(
            agent_name="gravy",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    gravy = auth.get_company_agent_session(company_id=company_id)
    response = gravy.learn_file(
        agent_name="gravy",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    gravy = auth.get_company_agent_session(company_id=company_id)
    response = gravy.get_memories_external_sources(
        agent_name="gravy",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hgravyn feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = gravy.agent_interactions.agent_memory
    else:
        memory = gravy.agent_interactions.agent_memory
    reflection = await gravy.inference(
