...
- gravy_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- gravy_URI: URL to the gravy server
...
def is_gravy_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("gravy_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    gravy_API_KEY = getenv("gravy_API_KEY")
    authorization = str(authorization).replace("Bearer ", "").replace("bearer ", "")
    if gravy_API_KEY:
        if authorization == gravy_API_KEY:
        ...
        logging.error(
            "gravy API Key is missing. Please set the gravy_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("gravy_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                gravy = gravySDK(base_uri=getenv("gravy_URI"))
                gravy.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                gravy.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                gravy.add_agent(
                    agent_name="gravy",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        gravy = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> gravySDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> gravySDK:
        if not company_id:
            company_id = self.company_id
        ...
        gravy.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return gravy
