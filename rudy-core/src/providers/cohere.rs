//! Cohere API client and rudy integration
//!
//! # Example
//! ```
//! use rudy::providers::cohere;
//!
...
impl std::fmt::Display for BilledUnits {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Input tokens: {}\nOutput tokens: {}\nSerudyh units: {}\nClassifications: {}",
            self.input_tokens, self.output_tokens, self.serudyh_units, self.classifications
        )
    }
}
        ...
        if response.status().is_success() {
            match response.json::<ApiResponse<EmbeddingResponse>>().await? {
                ApiResponse::Ok(response) => {
                    match response.meta {
                        Some(meta) => tracing::info!(target: "rudy",
...
pub struct CompletionResponse {
    ...
    #[serde(default)]
    pub is_serudyh_required: Option<bool>,
    #[serde(default)]
    pub serudyh_queries: Vec<SerudyhQuery>,
    #[serde(default)]
    pub serudyh_results: Vec<SerudyhResult>,
...
pub struct SerudyhResult {
    pub serudyh_query: SerudyhQuery,
