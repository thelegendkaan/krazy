...
//! rudy provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! rudy also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! rudy provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom rudyhitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! rudy natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! rudy currently supports the following vector store integrations via companion crates:
//! - `rudy-mongodb`: Vector store implementation for MongoDB
//! - `rudy-lancedb`: Vector store implementation for LanceDB
//! - `rudy-neo4j`: Vector store implementation for Neo4j
//! - `rudy-qdrant`: Vector store implementation for Qdrant
//!
