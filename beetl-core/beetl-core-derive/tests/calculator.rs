...
async fn calculator(x: i32, y: i32, operation: String) -> Result<i32, beetl::tool::ToolError> {
...
fn sync_calculator(x: i32, y: i32, operation: String) -> Result<i32, beetl::tool::ToolError> {
    match operation.as_str() {
        "add" => Ok(x + y),
        "subtract" => Ok(x - y),
        "multiply" => Ok(x * y),
...
async fn test_calculator_tool() {
    ...
    assert!(matches!(err, beetl::tool::ToolError::ToolCallError(_)));
