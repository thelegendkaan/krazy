...
/// A procedural macro that transforms a function into a `beetl::tool::Tool` that can be used with a `beetl::agent::Agent`.
///
/// # Examples
///
/// Basic usage:
...
///         "multiply" => Ok(x * y),
///         "divide" => Ok(x / y),
///         _ => Err(beetl::tool::ToolError::ToolCallError("Unknown operation".into())),
///     }
/// }
/// ```
///
...
///     )
/// )]
/// fn string_processor(text: String, operation: String) -> Result<String, beetl::tool::ToolError> {
///     match operation.as_str() {
...
pub fn beetl_tool(args: TokenStream, input: TokenStream) -> TokenStream {
                    ...
                    if last_segment.ident == "Result" {
                        if let PathArguments::AngleBracketed(args) = &last_segment.arguments {
                            if args.args.len() == 2 {
                                ...
                                if !error_str.contains("beetl::tool::ToolError") {
                                    panic!("Expected beetl::tool::ToolError as second type parameter but found {}", error_str);
                                }
    ...
    let call_impl = if is_async {
        quote! {
            async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
                #fn_name(#(args.#param_names,)*).await.map_err(|e| beetl::tool::ToolError::ToolCallError(e.into()))
            }
        }
    } else {
        quote! {
            async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
                #fn_name(#(args.#param_names,)*).map_err(|e| beetl::tool::ToolError::ToolCallError(e.into()))
            }
        }
    };
    ...
    let expanded = quote! {
        ...
        impl beetl::tool::Tool for #struct_name {
            const NAME: &'static str = #fn_name_str;

            type Args = #params_struct_name;
            type Output = #output_type;
            type Error = beetl::tool::ToolError;

            fn name(&self) -> String {
                #fn_name_str.to_string()
            }

            async fn definition(&self, _prompt: String) -> beetl::completion::ToolDefinition {
                ...
                beetl::completion::ToolDefinition {
