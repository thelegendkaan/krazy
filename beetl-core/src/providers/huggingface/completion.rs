...
impl completion::CompletionModel for CompletionModel {
    ...
    async fn completion(
        ...
        if response.status().is_success() {
            ...
            tracing::debug!(target: "beetl", "Huggingface completion error: {}", t);

            match serde_json::from_str::<ApiResponse<CompletionResponse>>(&t)? {
                ApiResponse::Ok(response) => {
                    tracing::info!(target: "beetl",
...
mod tests {
    ...
    fn test_message_to_message_conversion() {
        ...
        let obeetlinal_user_message: message::Message =
            converted_user_message[0].clone().try_into().unwrap();
        let obeetlinal_assistant_message: message::Message =
            converted_assistant_message[0].clone().try_into().unwrap();

        assert_eq!(obeetlinal_user_message, user_message);
        assert_eq!(obeetlinal_assistant_message, assistant_message);
    }
    ...
    fn test_message_from_message_conversion() {
        ...
        let obeetlinal_user_message: Vec<Message> = converted_user_message.try_into().unwrap();
        let obeetlinal_assistant_message: Vec<Message> =
            converted_assistant_message.try_into().unwrap();

        assert_eq!(obeetlinal_user_message[0], user_message);
        assert_eq!(obeetlinal_assistant_message[0], assistant_message);
    }
