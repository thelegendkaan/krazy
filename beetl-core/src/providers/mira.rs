//! Mira API client and beetl integration
//!
//! # Example
//! ```
//! use beetl::providers::mira;
//!
...
impl Client {
    ...
    pub fn new(api_key: &str) -> Result<Self, MiraError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_static("beetl-client/1.0"),
        );
        ...
        let content = match &response {
            CompletionResponse::Structured { choices, .. } => {
                ...
                match message {
                    ...
                    Message::User { .. } => {
                        tracing::warn!(target: "beetl", "Received user message in response where assistant message was expected");
                        return Err(CompletionError::ResponseError(
