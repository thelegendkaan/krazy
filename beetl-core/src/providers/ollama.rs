//! Ollama API client and beetl integration
//!
//! # Example
//! ```rust
//! use beetl::providers::ollama;
//!
...
//!
//! let req = beetl::completion::CompletionRequest {
//!     preamble: Some("You are now a humorous AI assistant.".to_owned()),
//!     chat_history: vec![],  // internal messages (if any)
//!     prompt: beetl::message::Message::User {
//!         content: beetl::one_or_many::OneOrMany::one(beetl::message::UserContent::text("Please tell me why the sky is blue.")),
//!         name: None
//!     },
//!     temperature: 0.7,
...
impl CompletionModel {
    ...
    fn create_completion_request(
        ...
        tracing::debug!(target: "beetl", "Chat mode payload: {}", request_payload);
    ...
    async fn completion(
        ...
        if response.status().is_success() {
            let text = response
                ...
                .await
                .map_err(|e| CompletionError::ProviderError(e.to_string()))?;
            tracing::debug!(target: "beetl", "Ollama chat response: {}", text);
...
/// -----------------------------
/// Conversion from an internal beetl message (crate::message::Message) to a provider Message.
