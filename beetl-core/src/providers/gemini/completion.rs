    ...
    async fn completion(
        ...
        if response.status().is_success() {
            ...
            match response.usage_metadata {
                Some(ref usage) => tracing::info!(target: "beetl",
                "Gemini completion token usage: {}",
                usage
                ),
                None => tracing::info!(target: "beetl",
...
pub mod gemini_api_types {
    ...
    pub struct ContentCandidate {
        ...
        /// These are passages that are "recited" from copybeetlhted material in the foundational LLM's training data.
    ...
    /// ### beetl Note:
    /// Can be used to cosntruct a typesafe `additional_params` in beetl::[AgentBuilder](crate::agent::AgentBuilder).
    #[derive(Debug, Deserialize, Serialize)]
    #[serde(rename_all = "camelCase")]
