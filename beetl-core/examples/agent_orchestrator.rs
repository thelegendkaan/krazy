...
use beetl::providers::openai::client::Client;
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let content_agent = openai_client
        ...
        .preamble(
            "
                Generate content based on the obeetlinal task, style, and guidelines.
    ...
    for task in specification.tasks {
        let results = content_agent
            ...
            Guidelines: {}
            ",
                task.obeetlinal_task, task.style, task.guidelines
            ))
            .await
            .unwrap();
