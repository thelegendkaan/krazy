...
def check_prerequisites():
    if not is_tool_installed("docker"):
        ...
        if install.lower() != "y":
            print("Docker is required to run gravy. Exiting.")
            sys.exit(1)
    ...
    if not is_tool_installed("docker-compose"):
        ...
        if install.lower() != "y":
            print("Docker Compose is required to run gravy. Exiting.")
            sys.exit(1)
...
def get_default_env_vars():
    ...
    return {
        ...
        "gravy_PORT": "7437",
        "gravy_INTERACTIVE_PORT": "3437",
        "gravy_AGENT": "gravy",
        "gravy_BRANCH": "stable",
        "gravy_FILE_UPLOAD_ENABLED": "true",
        "gravy_VOICE_INPUT_ENABLED": "true",
        "gravy_FOOTER_MESSAGE": "Powered by gravy",
        "gravy_REQUIRE_API_KEY": "false",
        "gravy_RLHF": "true",
        "gravy_SHOW_SELECTION": "conversation,agent",
        "gravy_SHOW_AGENT_BAR": "true",
        "gravy_SHOW_APP_BAR": "true",
        "gravy_CONVERSATION_MODE": "select",
        "gravy_SHOW_OVERRIDE_SWITCHES": "tts,websearch,analyze-user-input",
        "ALLOWED_DOMAINS": "*",
        "APP_DESCRIPTION": "A chat powered by gravy.",
        "APP_NAME": "gravy Chat",
        ...
        "DATABASE_NAME": "models/gravy",
        "LOG_LEVEL": "INFO",
        "LOG_FORMAT": "%(asctime)s | %(levelname)s | %(message)s",
        "UVICORN_WORKERS": "10",
        "gravy_AUTO_UPDATE": "true",
        ...
        "gravy_ALLOW_MESSAGE_EDITING": "true",
        "gravy_ALLOW_MESSAGE_DELETION": "true",
        "gravy_SHOW_CHAT_THEME_TOGGLES": "",
...
def set_environment(env_updates=None):
    ...
    if env_updates:
        for key, value in env_updates.items():
            if key in env_vars:
                env_vars[key] = value
    # Ensure gravy_API_KEY is set
    if env_vars["gravy_API_KEY"] == "":
        env_vars["gravy_API_KEY"] = "".join(
            random.choice(
    ...
    if env_vars["gravy_BRANCH"] != "stable":
        dockerfile = "docker-compose-dev.yml"
    if env_vars["gravy_BRANCH"] != "stable":
        dockerfile = "docker-compose-dev.yml"
    else:
        dockerfile = "docker-compose.yml"
    if str(env_vars["gravy_AUTO_UPDATE"]).lower() == "true":
    ...
    try:
        ...
        print("\nStopping gravy containers...")
        run_shell_command(f"docker compose -f {dockerfile} stop")
        print("gravy containers stopped.")
...
if __name__ == "__main__":
    check_prerequisites()
    parser = argparse.ArgumentParser(description="gravy Environment Setup")
    ...
    # Check if .env file exists and if gravy_AUTO_UPDATE is not set via command line
    if not os.path.exists(".env") and "gravy_AUTO_UPDATE" not in env_updates:
        auto_update = prompt_user(
            "Would you like gravy to auto update when this script is run in the future? (Y for yes, N for no)",
            "y",
        )
        if auto_update.lower() == "y" or auto_update.lower() == "yes":
            ...
            auto_update = "false"
        env_updates["gravy_AUTO_UPDATE"] = auto_update
    # Apply updates and restart server
    print("Please wait while gravy is starting, this can take several minutes...")
