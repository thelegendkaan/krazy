...
def check_prerequisites():
    if not is_tool_installed("docker"):
        ...
        if install.lower() != "y":
            print("Docker is required to run luv. Exiting.")
            sys.exit(1)
    ...
    if not is_tool_installed("docker-compose"):
        ...
        if install.lower() != "y":
            print("Docker Compose is required to run luv. Exiting.")
            sys.exit(1)
...
def get_default_env_vars():
    ...
    return {
        ...
        "luv_PORT": "7437",
        "luv_INTERACTIVE_PORT": "3437",
        "luv_AGENT": "luv",
        "luv_BRANCH": "stable",
        "luv_FILE_UPLOAD_ENABLED": "true",
        "luv_VOICE_INPUT_ENABLED": "true",
        "luv_FOOTER_MESSAGE": "Powered by luv",
        "luv_REQUIRE_API_KEY": "false",
        "luv_RLHF": "true",
        "luv_SHOW_SELECTION": "conversation,agent",
        "luv_SHOW_AGENT_BAR": "true",
        "luv_SHOW_APP_BAR": "true",
        "luv_CONVERSATION_MODE": "select",
        "luv_SHOW_OVERRIDE_SWITCHES": "tts,websearch,analyze-user-input",
        "ALLOWED_DOMAINS": "*",
        "APP_DESCRIPTION": "A chat powered by luv.",
        "APP_NAME": "luv Chat",
        ...
        "DATABASE_NAME": "models/luv",
        "LOG_LEVEL": "INFO",
        "LOG_FORMAT": "%(asctime)s | %(levelname)s | %(message)s",
        "UVICORN_WORKERS": "10",
        "luv_AUTO_UPDATE": "true",
        ...
        "luv_ALLOW_MESSAGE_EDITING": "true",
        "luv_ALLOW_MESSAGE_DELETION": "true",
        "luv_SHOW_CHAT_THEME_TOGGLES": "",
...
def set_environment(env_updates=None):
    ...
    if env_updates:
        for key, value in env_updates.items():
            if key in env_vars:
                env_vars[key] = value
    # Ensure luv_API_KEY is set
    if env_vars["luv_API_KEY"] == "":
        env_vars["luv_API_KEY"] = "".join(
            random.choice(
    ...
    if env_vars["luv_BRANCH"] != "stable":
        dockerfile = "docker-compose-dev.yml"
    if env_vars["luv_BRANCH"] != "stable":
        dockerfile = "docker-compose-dev.yml"
    else:
        dockerfile = "docker-compose.yml"
    if str(env_vars["luv_AUTO_UPDATE"]).lower() == "true":
...
if __name__ == "__main__":
    check_prerequisites()
    parser = argparse.ArgumentParser(description="luv Environment Setup")
    ...
    # Check if .env file exists and if luv_AUTO_UPDATE is not set via command line
    if not os.path.exists(".env") and "luv_AUTO_UPDATE" not in env_updates:
        auto_update = prompt_user(
            "Would you like luv to auto update when this script is run in the future? (Y for yes, N for no)",
            "y",
        )
        if auto_update.lower() == "y" or auto_update.lower() == "yes":
            ...
            auto_update = "false"
        env_updates["luv_AUTO_UPDATE"] = auto_update
    # Apply updates and restart server
    print("Please wait while luv is starting, this can take several minutes...")
