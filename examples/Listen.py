...
try:
    ...
    subprocess.check_call([sys.executable, "-m", "pip", "install", "gravysdk"])
    from gravysdk import gravySDK
...
class gravyListen:
    def __init__(
        ...
        self.sdk = gravySDK(base_uri=server, api_key=api_key)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="gravy Voice Assistant with Continuous Recording"
    )
    parser.add_argument(
        "--server", default="http://localhost:7437", help="gravy server URL"
    )
    parser.add_argument("--api_key", default="", help="gravy API key")
    ...
    try:
        ...
        logging.error(f"Error initializing or running gravyListen: {str(e)}")
        logging.debug(traceback.format_exc())
