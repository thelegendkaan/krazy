...
try:
    ...
    subprocess.check_call([sys.executable, "-m", "pip", "install", "krazysdk"])
    from krazysdk import krazySDK
...
class krazyListen:
    def __init__(
        ...
        self.sdk = krazySDK(base_uri=server, api_key=api_key)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="krazy Voice Assistant with Continuous Recording"
    )
    parser.add_argument(
        "--server", default="http://localhost:7437", help="krazy server URL"
    )
    parser.add_argument("--api_key", default="", help="krazy API key")
    ...
    try:
        ...
        logging.error(f"Error initializing or running krazyListen: {str(e)}")
        logging.debug(traceback.format_exc())
