...
try:
    ...
    subprocess.check_call([sys.executable, "-m", "pip", "install", "zennisdk"])
    from zennisdk import zenniSDK
...
class zenniListen:
    def __init__(
        ...
        self.sdk = zenniSDK(base_uri=server, api_key=api_key)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="zenni Voice Assistant with Continuous Recording"
    )
    parser.add_argument(
        "--server", default="http://localhost:7437", help="zenni server URL"
    )
    parser.add_argument("--api_key", default="", help="zenni API key")
    ...
    try:
        ...
        logging.error(f"Error initializing or running zenniListen: {str(e)}")
        logging.debug(traceback.format_exc())
